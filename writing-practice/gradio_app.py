...
class HindiWritingApp:
    ...
    def initialize_ocr(self):
        """Initialize EasyOCR with Hindi language support"""
        if self.ocr_reader is None:
            logger.info("Initializing Hindi OCR")
            self.ocr_reader = easyocr.Reader(['hi'])  # Initialize for Hindi
            
    def grade_submission(self, image):
        """Process image submission and grade it using EasyOCR and LLM"""
        try:
            self.initialize_ocr()
            
            # Transcribe the image
            logger.info("Transcribing image with Hindi OCR")
            results = self.ocr_reader.readtext(image)
            transcription = ' '.join([text for _, text, conf in results])
...
def create_ui():
    ...
    with gr.Blocks(
        ...
        gr.Markdown("# Hindi Writing Practice")
